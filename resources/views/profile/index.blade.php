            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="profile-bg-picture"
                        style="background-image:url('assets/images/_111073920_brarudibuja.jpg');background-size:contain">
                                            ...
                                            <div class="user-profile-content card p-3">
                                                <form method="POST" action="{{ route('users.store') }}"
                                                    ...
                                                    <div class="mb-6">
                                                        ...
                                                        <select onchange="triggerRole()" name="roles" id="roles"
                                                            class="form-control">
                                                    ...
                                                    <div class="mb-2" id="depotField" hidden>
                                                        <label class="form-label" for="RePassword">Depot</label>
                                                        <select name="depot" id="depotList" class="form-control">
                                                        </select>
                                                    </div>
...
@section('js_content')
    <script>
        ...
        function triggerRole() {
            let name = $('#roles option:selected').attr("data-name");
            $.ajax({
                url: "{{ route('getDePot') }}",
                type: "POST",
                data: {
                ...
                success: function(data) {
                    if (data.length > 0) {
                        $("#depotField").removeAttr("hidden")
                        $.each(data, function(index, value) {
                            $('#depotList').append($('<option></option>').val(value.id).html(value
                                .nom_depot))
                        })
                    } else {
                        $("#depotField").attr("hidden", true)
                    }
                },
